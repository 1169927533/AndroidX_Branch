 ...
 * @Description TODO
 */
class SSThree(val mContext: Context) : RecyclerView.LayoutManager() {
    var initLocationPosition = 2 //第一次可见的位置
    ...
    fun firstInitView(recycler: RecyclerView.Recycler) {
        for (index in 0 until itemCount) {
            ...
            var left = formerWidth * index + index * itemSpaceBetweenDistance.toInt()
            var top = 0
            ...
            var scale = getPositionScale(childView)
            //view的真实宽度不会随着缩放而变化
    ...
    fun onScrollLayoutView(recycler: RecyclerView.Recycler, dx: Int) {
        ...
        for (index in 0 until itemCount) {
            ...
            if (method) {
                ...
                if (scale > 1) {
                    left =
                        childView.left - dx - ((childView.left - childView.left * (1.7 - scale)) / 2f).toInt()
                } else {
                    left = childView.left - dx
                }
            }
     ...
     * 获取对应位置的缩放值
     */
    fun getPositionScale(view: View): Float {
